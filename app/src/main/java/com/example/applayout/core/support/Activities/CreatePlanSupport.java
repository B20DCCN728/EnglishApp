...
public class CreatePlanSupport extends AppCompatActivity {
    ...
    private void createPlan() {
        ...
        mDataBase.child(currentUser.getUid()).child("notes").push().setValue(note)
                .addOnSuccessListener(aVoid -> {
                    Toast.makeText(CreatePlanSupport.this, "Tạo kế hoạch thành công", Toast.LENGTH_SHORT).show();
                })
                .addOnFailureListener(e -> {
                    show_dialog("Tạo kế hoạch thất bại", 2);
                });
        Intent intent = new Intent(
                this,
                PlanSupport.class
    ...
    private void show_dialog(String s, int time){
        ProgressDialog progressDialog = new ProgressDialog(CreatePlanSupport.this);
